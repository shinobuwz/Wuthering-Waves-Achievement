        ...
        try
        {
            try
            {
                ...
                else
                {
                    ...
                    var normalizedMetadata = NormalizeTrackingMetadata(state.Metadata, mergedAchievements, statuses, out var trackingChanged);
                    if (metadataChanged || categoriesChanged || statuses.Count != state.Statuses.Count || trackingChanged)
                    {
