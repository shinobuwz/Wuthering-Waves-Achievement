...
try {
    ...
    if (-not $process.HasExited -and $process.MainWindowHandle -ne 0) {
        $required = @('ThemeButton','TrackSelectedButton','OpenTrackerButton','LegacyImportButton','ExchangeImportButton','ExchangeExportButton','WikiSyncButton','UpdateButton','AchievementGrid')
