...
提供外置 INI 配置文件，存储所有 OCR 扫描和游戏检测的可调参数，打包后用户可直接编辑调整。

## 需求

### 需求:外置配置文件定义
系统必须提供 `config.ini` 文件（exe 同级目录），以 INI 格式存储所有 OCR 扫描相关的可调参数，每个参数必须附带中文注释说明用途。
...
#### 场景:配置文件路径
- **当** 系统查找 config.ini
- **那么** 系统必须通过 `get_resource_path("config.ini")` 定位文件，即 exe 同级目录
