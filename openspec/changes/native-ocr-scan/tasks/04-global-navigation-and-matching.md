...
- `src/Wuwa.Core/OcrScanContracts.cs`
- `src/Wuwa.Core/OcrMatching.cs`
- `src/Wuwa.Infrastructure/WindowsGameWindowCapture.cs`
- `src/Wuwa.Infrastructure/NativeOcrClient.cs`
- `src/Wuwa.Infrastructure/NativeOcrTextReader.cs`
- `src/Wuwa.Infrastructure/NativeOcrTemplateTextReader.cs`
- `tests/Wuwa.Tests/`
...
- The new full-scan command follows the shipped primary-tab order, coordinate mapping, secondary-tab matching, and bounded scroll constants rather than introducing a new category or group data model.
- Primary-tab selection is verified by OCR after each click. A failed selection is retried using the Native retry budget; a category that still cannot be verified is reported and does not invalidate results already collected from other categories.
- Secondary tabs are matched against the ordered category names from the shipped achievement/category data. Each visible unvisited tab is clicked once. When no new tab is discovered, secondary navigation stops after a bounded retry count and reports any unvisited names.
...
2. Given a game window at the supported resolution and matching integrity level, full scan visits every verified primary/secondary category in the shipped category data, with no duplicate tab visits and bounded termination when navigation stops producing new tabs.
...
5. Accepted full-scan results use the existing preview and `AchievementWorkspace` merge path, preserve completed-status downgrade protection, and do not alter legacy progress files.
...
- `dotnet test WutheringWavesAchievement.sln -c Release --no-restore`, native OCR CTest, and `dotnet build WutheringWavesAchievement.sln -c Release --no-restore`.
